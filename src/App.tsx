...
import { format, addDays, isBefore, isAfter, parseISO, differenceInDays } from 'date-fns';
import { 
  BarChart, 
  Bar, 
  XAxis, 
...
const DashboardView = ({ products, batches, sales, salesByDay, onNavigate }: { products: Product[], batches: Batch[], sales: Sale[], salesByDay: any[], onNavigate: (view: any, filter?: any) => void }) => {
    ...
    <div className="space-y-6 pb-12">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="p-4 bg-gradient-to-br from-primary/10 to-white border-primary/20">
          ...
          <p className="text-[10px] text-slate-500 mt-1">Last 100 transactions</p>
        </Card>
        ...
        <button onClick={() => onNavigate('inventory', { filter: 'expired' })} className="text-left">
          <Card className="p-4 hover:shadow-md transition-all border-red-100 bg-red-50/30">
            <div className="flex items-center justify-between">
              <p className="text-xs font-bold text-red-600 uppercase tracking-wider">Expired Drugs</p>
              <XCircle className="w-4 h-4 text-red-500" />
            </div>
            <p className="text-2xl font-bold mt-2 text-red-700">{expired.length}</p>
            <p className="text-[10px] text-red-600 mt-1">Requires disposal</p>
          </Card>
        </button>
      </div>
...
const InventoryView = ({ products, batches, businessId, initialFilter }: { products: Product[], batches: Batch[], businessId: string, initialFilter?: string }) => {
  ...
  const handleDeleteProduct = async (id: string) => {
    if (confirm('Are you sure you want to delete this product? All associated batches will also be affected.')) {
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <Input name="barcode" defaultValue={editingProduct?.barcode} placeholder="Scan or enter barcode" />
                  ...
                  <div>
                    ...
                    <Input name="imageUrl" defaultValue={editingProduct?.imageUrl} placeholder="https://example.com/image.jpg" />
                  </div>
                </div>
...
const POSView = ({ products, batches, businessId, user }: { products: Product[], batches: Batch[], businessId: string, user: FirebaseUser }) => {
  ...
  const addToCart = (product: Product, unit: 'unit' | 'pack' | 'piece' = 'unit') => {
    ...
    if (existing) {
      ...
      const batch = batches.find(b => b.productId === product.id && isAfter(parseISO(b.expirationDate), new Date()));
      if (!batch) {
  ...
  const resolveInteraction = (action: 'remove' | 'override' | 'replace') => {
    if (action === 'remove') {
      ...
      if (interactionAlert?.alternatives) {
        // Extract first alternative name if possible, or just use the whole string
        const alt = interactionAlert.alternatives.split(',')[0].trim();
        setSearchTerm(alt);
        setPosStep('browsing');
        ...
        <div className="relative flex-1 min-h-0 overflow-hidden">
          <div className={cn(
              ...
              <Card className="flex-1 overflow-y-auto p-4 bg-slate-50/30 border-slate-200">
                <div className={cn(
                  ...
                  {filteredProducts.map(product => (
                    <div
                      key={product.id}
                      className="flex flex-col p-4 rounded-2xl bg-white border border-slate-200 hover:border-primary hover:shadow-lg transition-all text-left group relative overflow-hidden"
                    >
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <span className="text-[10px] font-bold text-primary uppercase block truncate">{product.category}</span>
                      ...
                      <div className="space-y-2 mt-auto">
                        {product.sellType === 'pack_piece' ? (
                          <div className="grid grid-cols-2 gap-2">
                            <button 
                              ...
                              className="flex flex-col items-center p-2 rounded-xl bg-slate-50 hover:bg-primary/10 hover:border-primary border border-transparent transition-all"
                            >
                              <span className="text-[10px] font-bold text-slate-500 uppercase">Pack</span>
                              <span className="text-xs font-bold text-slate-900">₦{product.sellingPrice.toLocaleString()}</span>
                            </button>
                            <button 
                              onClick={() => addToCart(product, 'piece')}
                              className="flex flex-col items-center p-2 rounded-xl bg-slate-50 hover:bg-primary/10 hover:border-primary border border-transparent transition-all"
                            >
                              <span className="text-[10px] font-bold text-slate-500 uppercase">Piece</span>
                              <span className="text-xs font-bold text-slate-900">₦{product.sellingPricePerPiece?.toLocaleString()}</span>
                            </button>
                          </div>
                        ) : (
                          <button 
                            onClick={() => addToCart(product, 'unit')}
                            className="w-full flex items-center justify-between p-3 rounded-xl bg-slate-50 hover:bg-primary/10 hover:border-primary border border-transparent transition-all"
                          >
                            <span className="text-xs font-bold text-slate-900">₦{product.sellingPrice.toLocaleString()}</span>
                        ...
                        <div className="flex items-center justify-between px-1">
                          <span className={cn(
                            ...
                            {product.sellType === 'pack_piece' ? (
                              `${Math.floor(product.totalQuantity / (product.piecesPerPack || 1))}p, ${product.totalQuantity % (product.piecesPerPack || 1)}pc left`
                            ) : (
                              `${product.totalQuantity} units left`
                            )}
                          </span>
                        </div>
                      </div>
            ...
            <AnimatePresence mode="wait">
              {posStep !== 'browsing' && posStep !== 'safety_check' && (
                <motion.div 
                  key={posStep}
                  ...
                  className="flex flex-col space-y-4 min-h-0 h-full"
                >
                  <Card className="flex-1 flex flex-col p-0 bg-white border-slate-200 shadow-2xl relative overflow-hidden">
                    {/* Header */}
                      ...
                      {posStep === 'cart' && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            ...
                            <Button 
                              className="h-16 text-sm font-black uppercase tracking-widest shadow-lg shadow-primary/20"
            ...
            {/* Full-Page Safety Check Overlay */}
            <AnimatePresence>
              {posStep === 'safety_check' && (
                <motion.div
                  initial={{ y: '100%' }}
                  animate={{ y: 0 }}
                  exit={{ y: '100%' }}
                ...
                >
                  <div className="h-16 border-b border-slate-100 flex items-center justify-between px-6 bg-white sticky top-0 z-10">
                    <div className="flex items-center gap-3">
                      ...
                      <h2 className="text-sm font-black uppercase tracking-widest text-slate-900">Safety Check</h2>
                    </div>
                    <button 
                      onClick={() => setPosStep('browsing')}
                  ...
                  <div className="flex-1 overflow-y-auto p-6 md:p-12 max-w-4xl mx-auto w-full">
                    <div className="space-y-8">
                      <div className="text-center space-y-2">
                        <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tight">Interaction Review</h3>
                        <p className="text-slate-500 text-sm">Mandatory safety screening for the current cart contents.</p>
                      </div>

                      {isCheckingInteractions ? (
                        <div className="flex flex-col items-center justify-center py-20 space-y-4">
                          <Loader2 className="w-12 h-12 text-primary animate-spin" />
                          <p className="text-sm font-bold text-slate-400 uppercase tracking-widest animate-pulse">Analyzing Drug Interactions...</p>
                        </div>
                      ) : interactionWarnings.length === 0 ? (
                        <div className="bg-emerald-50 border border-emerald-100 rounded-3xl p-12 text-center space-y-6">
                          <div className="w-20 h-20 bg-emerald-100 rounded-full flex items-center justify-center mx-auto">
                            <CheckCircle2 className="w-10 h-10 text-emerald-600" />
                          </div>
                          <div className="space-y-2">
                            <h4 className="text-xl font-bold text-emerald-900 uppercase">All Clear</h4>
                            <p className="text-emerald-700 text-sm">No significant drug-drug interactions detected for this combination.</p>
                          </div>
                          <Button 
                            className="w-full h-14 text-lg font-black uppercase tracking-widest bg-emerald-600 hover:bg-emerald-700 shadow-xl shadow-emerald-200"
                            onClick={() => setPosStep('cart')}
                          >
                            Proceed to Cart
                          </Button>
                        ...
                        <div className="space-y-6">
                          <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4 flex items-start gap-3">
                            ...
                            <p className="text-xs text-amber-800 font-medium leading-relaxed">
                              <span className="font-bold uppercase">Warning:</span> Potential interactions detected. Please review and resolve each warning before proceeding.
                            </p>
                          </div>
                                ...
                                <Card className="p-6 border-l-4 border-l-red-500 shadow-sm hover:shadow-md transition-shadow">
                                  <div className="space-y-4">
                                    ...
                                    <div className="space-y-1">
                                      <div className="flex items-center gap-2">
                                        <span className="px-2 py-0.5 bg-red-100 text-red-700 text-[10px] font-black uppercase rounded">High Risk</span>
                                        <h4 className="font-bold text-slate-900 uppercase tracking-tight">{warning.drugA} + {warning.drugB}</h4>
                                      </div>
                                      <p className="text-sm text-slate-600 leading-relaxed">{warning.description}</p>
                                    </div>
                                  </div>

                                  <div className="bg-slate-50 rounded-xl p-4 space-y-2">
                                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Recommendation</p>
                                    <p className="text-xs text-slate-700 font-medium italic">"{warning.recommendation}"</p>
                                  </div>
                                  ...
                                  <div className="grid grid-cols-2 gap-3 pt-2">
                                    <Button 
                                      variant="outline" 
                                      size="sm" 
                                      className="text-[10px] font-bold uppercase h-10 border-slate-200 hover:bg-red-50 hover:text-red-600 hover:border-red-200"
                                      onClick={() => {
                                        setCart(prev => prev.filter(item => item.name !== warning.drugA && item.name !== warning.drugB));
                                    ...
                                    <Button 
                                      ...
                                      onClick={() => {
                                        ...
                                        if (reason) {
                                          ...
                                          setOverrideLogs(prev => [...prev, {
                                            timestamp: new Date(),
                                            drugs: [warning.drugA, warning.drugB],
                                            reason
                                          }]);
                                        }
                                      }}
                          ...
                          {interactionWarnings.length === 0 && (
                            <Button 
                              className="w-full h-14 text-lg font-black uppercase tracking-widest shadow-xl shadow-primary/30"
                              onClick={() => setPosStep('cart')}
                            >
                              Proceed to Cart
                            </Button>
...
const LedgerView = ({ sales, businessId }: { sales: Sale[], businessId: string }) => {
  const creditSales = useMemo(() => 
    sales.filter(s => s.paymentMethod === 'credit' && s.customerDetails?.amountOwed && s.customerDetails.amountOwed > 0)
  , [sales]);

  const handleMarkAsPaid = async (saleId: string, currentAmount: number) => {
    if (!confirm(`Mark ₦${currentAmount.toLocaleString()} as paid?`)) return;
    try {
      await updateDoc(doc(db, 'sales', saleId), {
        'customerDetails.amountOwed': 0,
        'paymentMethod': 'cash', // Convert to cash once paid
                  ...
                  <tr key={sale.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <Button 
                        ...
                        onClick={() => handleMarkAsPaid(sale.id!, sale.customerDetails!.amountOwed!)}
                      >
...
const SettingsView = ({ business, businessId, user }: { business: Business | null, businessId: string, user: FirebaseUser }) => {
  ...
  const connectPrinter = async () => {
    ...
    try {
      ...
      const device = await (navigator as any).bluetooth.requestDevice({
        filters: [{ services: ['000018f0-0000-1000-8000-00805f9b34fb'] }], // Common thermal printer service UUID
            ...
            <div className="mt-10 pt-10 border-t border-slate-100">
              ...
              <div className="p-6 rounded-2xl bg-slate-50 border border-slate-200">
                ...
                <p className="text-xs text-slate-500 leading-relaxed">
                  Turn on your thermal receipt printer and ensure Bluetooth is enabled. Click to pair. 
                  Note: Web Bluetooth requires a secure connection and a compatible browser (Chrome/Edge).
                </p>
                {printerError && (
        ...
        <div className="space-y-8">
          <Card className="p-8 bg-primary text-white border-none shadow-2xl shadow-primary/20 relative overflow-hidden group">
            ...
            <div className="relative z-10">
              ...
              <Button variant="secondary" className="w-full h-12 bg-white text-primary hover:bg-slate-50 border-none font-bold">
                Upgrade to Premium
              </Button>
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <Card className="p-6 lg:col-span-2">
          <h3 className="text-sm font-bold mb-6 text-slate-500 uppercase tracking-wider">Sales Trend</h3>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={sales.slice(-7).map(s => ({ date: format(s.timestamp?.toDate() || new Date(), 'MMM dd'), amount: s.total }))}>
                <defs>
                ...
                />
                <Area type="monotone" dataKey="amount" stroke="#2563eb" strokeWidth={2} fillOpacity={1} fill="url(#colorTotal)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </Card>
        ...
        <Card className="p-6">
          <h3 className="text-sm font-bold mb-6 text-slate-500 uppercase tracking-wider">Sales by Category</h3>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  ...
                  dataKey="value"
                >
                  {['#2563eb', '#10b981', '#f59e0b', '#ef4444'].map((color, index) => (
          ...
          <div className="mt-4 space-y-2">
            {Object.entries(
              ...
              <div key={cat} className="flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full" style={{ backgroundColor: ['#2563eb', '#10b981', '#f59e0b', '#ef4444'][i % 4] }} />
      ...
      <Card>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                <th className="px-6 py-3 font-bold text-slate-500 uppercase text-[10px]">Date</th>
                <th className="px-6 py-3 font-bold text-slate-500 uppercase text-[10px]">Items</th>
                <th className="px-6 py-3 font-bold text-slate-500 uppercase text-[10px]">Method</th>
                <th className="px-6 py-3 font-bold text-slate-500 uppercase text-[10px] text-right">Total</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {sales.slice(0, 10).map(sale => (
                <tr key={sale.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4 text-slate-500">
                    {sale.timestamp ? format(sale.timestamp.toDate(), 'MMM dd, HH:mm') : 'Pending...'}
                  </td>
                  <td className="px-6 py-4">
                    <p className="font-bold text-slate-900">{sale.items.length} items</p>
    ...
    <div className="min-h-screen bg-zinc-50 flex">
      ...
      <aside className={cn(
        ...
        <div className="p-6 flex items-center gap-3">
          ...
          {isSidebarOpen && <span className="font-bold text-lg truncate">PI TRACK IT</span>}
        </div>
        ...
        <div className="p-8 max-w-7xl mx-auto w-full">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {currentView === 'ledger' && (
                <LedgerView sales={sales} businessId={business.id} />
              )}
              {currentView === 'reports' && <ReportsView sales={sales} products={products} />}
              {currentView === 'settings' && (
                <SettingsView business={business} businessId={business.id} user={user!} />
              )}
            </motion.div>
          </AnimatePresence>
        </div>
